...
alexsandr = np.array([183-135, 83-66]) #(83 кг), (183 см)
print("Эмили: %.3f" % network.feedforward(emily)) # 0.951 - Ж
print("Фрэнк: %.3f" % network.feedforward(frank)) # 0.039 - М
print("Александр: %.3f" % network.feedforward(alexsandr))


y_true = np.array([1, 0, 0])
y_pred = np.array([round(network.feedforward(emily)), round(network.feedforward(frank)), round(network.feedforward(alexsandr))])
print(f'Величина потерь в предсказаниях {mse_loss(y_true, y_pred)}')